...
describe('Page: RegisterPage', () => {
  ...
  it('debería mostrar error si el email ya existe', async () => {
    localStorageMock.setItem('mitienda_users', JSON.stringify([{ email: 'existe@test.com', password: 'p' }]));
    renderRegisterPage();
    
    fireEvent.change(screen.getByLabelText(/Nombre/i), { target: { value: 'Usuario Existente' } });
    fireEvent.change(screen.getByLabelText(/Contraseña/i), { target: { value: 'password123' } });
    fireEvent.change(screen.getByLabelText(/Email/i), { target: { value: 'existe@test.com' } });
